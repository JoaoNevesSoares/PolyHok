  ...
  do {                                                                         \
    ...
    if (err__ != CUDA_SUCCESS) {                                               \
      ...
      fprintf(stderr, "CUDA driver error at %s:%d: %s, %s\n", __FILE__,        \
              __LINE__, name ? name : "unknown", msg ? msg : "no message");    \
      exit(EXIT_FAILURE);                                                      \
    }                                                                          \
  } while (0)
...
void init_cuda(ErlNifEnv *env) {
  if (context == NULL) {
    ...
    CUDA_DRV_CHECK(cuCtxCreate(&context, nullptr, 0, device));

  } else {
    CUDA_DRV_CHECK(cuCtxSetCurrent(context));
  }
}
...
#define MX_LENGTH(matrix)                                                      \
  ((((uint32_t *)matrix)[0]) * (((uint32_t *)matrix)[1]) + 2)
...
static int load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {
  ...
  KERNEL_TYPE = enif_open_resource_type(env, NULL, "kernel", NULL,
                                        ERL_NIF_RT_CREATE, NULL);
  ARRAY_TYPE = enif_open_resource_type(
      env, NULL, "gpu_ref", dev_array_destructor, ERL_NIF_RT_CREATE, NULL);
  PINNED_ARRAY = enif_open_resource_type(env, NULL, "pinned_array",
                                         dev_pinned_array_destructor,
                                         ERL_NIF_RT_CREATE, NULL);
  return 0;
}
...
char *compile_to_ptx(ErlNifEnv *env, char *program_source) {
  ...
  nvrtcProgram prog;
  rv = nvrtcCreateProgram(&prog, program_source, nullptr, 0, nullptr, nullptr);
  if (rv != NVRTC_SUCCESS)
...
static ERL_NIF_TERM jit_compile_and_launch_nif(ErlNifEnv *env, int argc,
  ...
  init_cuda(env);
  CUDA_DRV_CHECK(cuModuleLoadDataEx(&module, ptx, 0, 0, 0));
  CUDA_DRV_CHECK(cuModuleGetFunction(&function, module, kernel_name));
  CUcontext current_after_getfunc;
  CUDA_DRV_CHECK(cuCtxGetCurrent(&current_after_getfunc));
  ...
  for (int i = 0; i < size_args; i++) {
    ...
    if (strcmp(type_name, "int") == 0) {
      ...
      if (!enif_get_resource(env, head_args, ARRAY_TYPE, (void **)&array_res)) {
        printf("FAILED enif_get_resource for arg %d\n", i);
        return enif_make_badarg(env);
      }

      printf("arg %d type=%s gpu_ptr=%llu\n", i, type_name,
             (unsigned long long)(*array_res));

      CUcontext ptr_ctx;
      CUresult attr_err = cuPointerGetAttribute(
          &ptr_ctx, CU_POINTER_ATTRIBUTE_CONTEXT, *array_res);

      if (attr_err != CUDA_SUCCESS) {
        const char *error;
        cuGetErrorString(attr_err, &error);
        printf("cuPointerGetAttribute failed for arg %d: %s\n", i, error);
      } else {
        CUcontext current;
        cuCtxGetCurrent(&current);

        printf("arg %d ptr_ctx=%p current_ctx=%p global_ctx=%p\n", i, ptr_ctx,
               current, context);
      }
...
static ERL_NIF_TERM get_gpu_array_nif(ErlNifEnv *env, int argc,
                                      const ERL_NIF_TERM argv[]) {
  int nrow;
  int ncol;
  char type_name[1024];
  ...
  if (strcmp(type_name, "float") == 0) {
    ...
    float *result_data =
        (float *)enif_make_new_binary(env, result_size, &result);
    ...
    if (err != CUDA_SUCCESS) {
      ...
      strcpy(message, "Error (get_gpu_array_nif): error compying data from "
                      "device to host: ");
    ...
    if (err != CUDA_SUCCESS) {
      ...
      cuGetErrorString(err, &error);
      strcpy(message,
             "Error (get_gpu_array_nif) copying data from device to host: ");
      strcat(message, error);
      enif_raise_exception(env, enif_make_string(env, message, ERL_NIF_LATIN1));
    }
    //////// END CUDA CALL
    ...
    double *result_data =
        (double *)enif_make_new_binary(env, result_size, &result);
...
static ERL_NIF_TERM create_gpu_array_random_nx_nif(ErlNifEnv *env, int argc,
  ...
  if (!enif_get_int(env, argv[1], &high)) {
    return enif_make_badarg(env);
  }
...
static ERL_NIF_TERM create_gpu_array_nx_nif(ErlNifEnv *env, int argc,
                                            const ERL_NIF_TERM argv[]) {
  ...
  *gpu_res = dev_array;
  ...
  // ...and release the resource so that it will be freed when Erlang garbage
  // collects
  enif_release_resource(gpu_res);
  return term;
}

static ERL_NIF_TERM new_gpu_array_nif(ErlNifEnv *env, int argc,
                                      const ERL_NIF_TERM argv[]) {
  size_t data_size;
  int nrow, ncol;
  ERL_NIF_TERM term;
...
static ERL_NIF_TERM new_pinned_nif(ErlNifEnv *env, int argc,
                                   const ERL_NIF_TERM argv[]) {
  ...
  float **pinned_res =
      (float **)enif_alloc_resource(PINNED_ARRAY, sizeof(float *));
...
static ERL_NIF_TERM new_gmatrex_pinned_nif(ErlNifEnv *env, int argc,
                                           const ERL_NIF_TERM argv[]) {
  float *matrix;
  float *dev_matrix;
  cudaError_t error_gpu;
...
static ERL_NIF_TERM create_nx_ref_nif(ErlNifEnv *env, int argc,
                                      const ERL_NIF_TERM argv[]) {
  ...
  if (strcmp(type_name, "float") == 0) {
    ...
    float **gpu_res =
        (float **)enif_alloc_resource(ARRAY_TYPE, sizeof(float *));
    *gpu_res = dev_matrix;
    term = enif_make_resource(env, gpu_res);
    // ...and release the resource so that it will be freed when Erlang garbage
    // collects
    ...
    *gpu_res = dev_matrix;
    ...
    double **gpu_res =
        (double **)enif_alloc_resource(ARRAY_TYPE, sizeof(double *));
    *gpu_res = dev_matrix;
    term = enif_make_resource(env, gpu_res);
    // ...and release the resource so that it will be freed when Erlang garbage
    // collects
...
static ERL_NIF_TERM create_ref_nif(ErlNifEnv *env, int argc,
                                   const ERL_NIF_TERM argv[]) {
  ErlNifBinary matrix_el;
  float *matrix;
  float *dev_matrix;
...
static ERL_NIF_TERM new_gpu_nx_nif(ErlNifEnv *env, int argc,
                                   const ERL_NIF_TERM argv[]) {
  int data_size;
  cudaError_t error_gpu;
  int nrow, ncol;
...
static ERL_NIF_TERM new_ref_nif(ErlNifEnv *env, int argc,
                                const ERL_NIF_TERM argv[]) {
  float *dev_matrix;
  int data_size;
  cudaError_t error_gpu;
...
static ERL_NIF_TERM get_nx_nif(ErlNifEnv *env, int argc,
                               const ERL_NIF_TERM argv[]) {
  int nrow;
  int ncol;
  ERL_NIF_TERM result;
...
static ERL_NIF_TERM get_matrex_nif(ErlNifEnv *env, int argc,
                                   const ERL_NIF_TERM argv[]) {
  int nrow;
  int ncol;
  ERL_NIF_TERM result;
...
static ERL_NIF_TERM synchronize_nif(ErlNifEnv *env, int argc,
                                    const ERL_NIF_TERM argv[]) {
  cudaError_t error_gpu;
...
static ERL_NIF_TERM load_kernel_nif(ErlNifEnv *env, int argc,
                                    const ERL_NIF_TERM argv[]) {
  ...
  enif_get_string(env, e_name_fun, kernel_name, size_name_fun + 1,
                  ERL_NIF_LATIN1);
  enif_get_string(env, e_name_module, module_name, size_name_module + 1,
                  ERL_NIF_LATIN1);
  ...
  if (m_handle == NULL) {
    ...
    strcpy(
        message,
        "Error opening shared library for the programa. It was not found.\n");
    enif_raise_exception(env, enif_make_string(env, message, ERL_NIF_LATIN1));
    return enif_make_int(env, 0);
  }
  ...
  void (**kernel_res)() =
      (void (**)())enif_alloc_resource(KERNEL_TYPE, sizeof(void *));
...
static ERL_NIF_TERM load_fun_nif(ErlNifEnv *env, int argc,
                                 const ERL_NIF_TERM argv[]) {
...
static ERL_NIF_TERM spawn_nif(ErlNifEnv *env, int argc,
                              const ERL_NIF_TERM argv[]) {
  void (**kernel_res)(ErlNifEnv *env, const ERL_NIF_TERM argv[],
                      ErlNifResourceType *type, ErlNifResourceType *ftype);
  // void (**kernel_res)();
  // float **array_res;
  // printf("spawn begin\n");
  // fflush(stdout);
  ...
  void (*fn)(ErlNifEnv *env, const ERL_NIF_TERM argv[],
             ErlNifResourceType *type, ErlNifResourceType *ftype) = *kernel_res;
  // void (*fn)() = *kernel_res;
