...
static ERL_NIF_TERM fail_cuda(ErlNifEnv *env, CUresult result, const char *obs) {
    char message[1000];
    const char *error;
...
static ERL_NIF_TERM jit_compile_and_launch_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
 ...
 // err = cuCtxCreate(&context2, 0, device);
  ...
  // printf("after module load err=%d module=%p\n", (int)err, module);

if (err != CUDA_SUCCESS) {
    return fail_cuda(env, err, "cuModuleLoadDataEx");
}

err = cuModuleGetFunction(&function, module, kernel_name);
printf("after get function err=%d module=%p function=%p kernel=%s\n",
       (int)err, module, function, kernel_name);

if (err != CUDA_SUCCESS) {
    return fail_cuda(env, err, "cuModuleGetFunction");
}

CUcontext current_after_getfunc;
cuCtxGetCurrent(&current_after_getfunc);

printf("after get function current_ctx=%p global_ctx=%p\n",
       current_after_getfunc, context);
...
printf("before launch module=%p function=%p current_ctx=%p global_ctx=%p\n",
       module, function, current_before_launch, context);
...
printf("after kernel launch err=%d\n", (int)err);

if (err != CUDA_SUCCESS) {
    return fail_cuda(env, err, "cuLaunchKernel");
}

err = cuCtxSynchronize();

if (err != CUDA_SUCCESS) {
    return fail_cuda(env, err, "cuCtxSynchronize");
}
...
static ERL_NIF_TERM create_gpu_array_random_nx_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  init_cuda(env);
  CUdeviceptr dev_array;
  CUresult err = cuMemAlloc(&dev_array, sizeof(float) * n);
