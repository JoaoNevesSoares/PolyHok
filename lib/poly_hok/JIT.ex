...
defmodule JIT do
  def compile_function({:anon, fname, code, type}) do
    ...
    cuda_body =
      PolyHok.CudaBackend.gen_cuda_jit(body, inf_types, param_vars, "module", MapSet.new())
  ...
  def infer_types_actual_parameters([h | t]) do
    case h do
      ...
      {:closure, _name, _ast, _args, _param} ->
        [:none | infer_types_actual_parameters(t)]
  ...
  def module_server(types_map, ast_map, fusion_map) do
    receive do
      {:add_function, fun_name, fun} ->
        module_server(types_map, Map.put(ast_map, fun_name, fun), fusion_map)

      {:add_ast, fun, ast, funs} ->
        module_server(types_map, Map.put(ast_map, fun, {ast, funs}), fusion_map)

      {:get_ast, f_name, pid} ->
        send(pid, {:ast, ast_map[f_name]})
        module_server(types_map, ast_map, fusion_map)
      ...
      {:add_type, fun, type} ->
        module_server(Map.put(types_map, fun, type), ast_map, fusion_map)
      ...
      {:fusion_map, pid} ->
        send(pid, {:fusion_map, fusion_map})
        module_server(types_map, ast_map, fusion_map)

      {:put_fusion, key, value} ->
        module_server(types_map, ast_map, Map.put(fusion_map, key, value))

      {:add_include, inc} ->
        case ast_map[:include] do
          nil -> module_server(types_map, Map.put(ast_map, :include, [inc]), fusion_map)
          l -> module_server(types_map, Map.put(ast_map, :include, [inc | l]), fusion_map)
        end
  ...
  defp process_definitions(module_name, [h | t], l) do
    case h do
      ...
      _ ->
        process_definitions_include(module_name, h, t, l)
    end
  end
  ...
  defp normalize_called_function({:., _, [_module_ast, fun_name]}) when is_atom(fun_name),
    do: fun_name
